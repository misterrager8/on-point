    ...
    <>
      <div
        className={
          "task-item" +
          (item.done ? " done" : "") +
          (item.id === multiCtx.currentTask?.id ? " active" : "")
        }>
        <div className="d-flex text-truncate">
          ...
          <div
            onClick={() => {
              !item.done &&
                multiCtx.setCurrentTask(
                  multiCtx.currentTask?.id === item.id ? null : item,
                );
            }}
      ...
      {multiCtx.currentTask && multiCtx.currentTask?.id === item.id && <Task />}
    </>
  );
}
